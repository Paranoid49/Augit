...
本报告不是首版完成声明。Windows 11 管理员安装、覆盖升级和卸载验收已经完成，安装器未更新已有的兼容运行时，并在卸载后自行恢复全部安装痕迹；Windows 10 22H2 当前没有可用测试环境，仍需完成同等验收。
...
- 当前机器实际使用 .NET Runtime `10.0.5`，满足正式产物声明的 `10.0.0` 兼容下限；Windows 11 管理员验收确认安装器不会为兼容运行时强制下载锁定版 `10.0.11`。Windows 10 22H2 仍需单独验证该行为。
- Windows 10 22H2 验收完成前，阶段五和首版发布条件均保持未完成。
