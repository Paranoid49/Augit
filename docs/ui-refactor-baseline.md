...
真实外壳中文件历史底部面板**尚未取得可确认的截图**。原因有二：
1. `--width/--height` 在提交版外壳中不存在，窗口尺寸由设置文件决定，
   底部面板在默认高度下被挤到可视区之外；
2. `MainWindowHandle` 在窗口构造期间可能指向非主窗口，导致截图尺寸异常。
下一轮应给外壳补上 `--width/--height`（或固定审计窗口尺寸），再对底部面板取证。
