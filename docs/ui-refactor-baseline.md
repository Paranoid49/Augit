...
- 真实外壳中 Blame 视图仍显示样例内容。浏览器验收套件（27 项断言）与后端探针都通过，
  说明数据层与渲染逻辑正确，问题出在外壳运行时；用于定位的日志手段（写临时目录、
  写仓库目录、读 stderr）本轮多次失效，未能取得证据。下一轮需先建立可靠的外壳内取证方式。
