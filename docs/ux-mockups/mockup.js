...
function renderScene() {
  ...
  switch (scene) {
    ...
    case "settings": return shell({ activeRail: "project", side: "project", editor: "markdown", overlay: dialog("设置 — Augit", settingsBody, `<a class="secondary-button" href="main-project.html">取消</a><button class="secondary-button">应用</button><a class="primary-button" href="main-project.html">确定</a>`, true, "dialog-xl") });
    ...
    case "rollback": return shell({ activeRail: "commit", side: "commit", editor: "diff", overlay: dialog("回滚文件 · src/Augit.App/app.manifest", `<div class="inline-alert danger rollback-impact"><strong>将丢失此文件的全部本地改动</strong><p class="commit-meta">回滚完整文件，不能只回滚选中的差异块。</p><p class="rollback-recycle" hidden>未跟踪或新增文件将移入 Windows 回收站。</p></div><div class="rollback-comparison">${diffView(true, "ready", true)}</div>`, `<a class="secondary-button" href="commit-diff.html">取消</a><a class="danger-button" href="operation-progress.html">回滚完整文件</a>`, true, "rollback-dialog") });
